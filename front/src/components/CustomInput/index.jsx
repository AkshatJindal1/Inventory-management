...
class CustomInput extends Component {
    ...
    componentWillMount() {
        ...
        const { isError } = this.validateUtil(
            value,
            condition,
            required,
            datatype
    ...
    componentDidUpdate(prevProps) {
        ...
        if (
            this.props.validateNow !== prevProps.validateNow &&
            this.props.validateNow === true
        ) {
            this.validate(
                prevProps.value,
                prevProps.condition,
    ...
    validate = (value, condition, required, datatype) => {
        const { isError, errorText } = this.validateUtil(
            value,
            condition,
            required,
            datatype
    ...
    validateUtil = (value, condition, required, datatype) => {
        ...
        if (validator.isEmpty(value + '')) {
            ...
            let minMaxCondition = {}
            if (condition.min)
                minMaxCondition = { ...minMaxCondition, min: condition.min }
            if (condition.max)
                minMaxCondition = { ...minMaxCondition, max: condition.max }
    ...
    render() {
        ...
        if (menuitems) {
            const menu = menuitems.map((m, index) => {
                return (
                    <MenuItem key={index} value={m.id} name={id}>
                        {m.labelText}
                    </MenuItem>
                )
            })
