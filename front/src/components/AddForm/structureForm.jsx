...
export class StructureForm extends Component {
    ...
    handleChange = (event, index, root) => {
        ...
        if (formFields[index].conditions === undefined)
            formFields[index].conditions = {}
        if (root === 'conditions')
            formFields[index].conditions[key] = event.target.value
        if (root === 'root') {
            if (key === 'required')
                formFields[index][key] = event.target.value === 1 ? true : false
    ...
    getOption = (datatype, defaultCondition, required, formIndex) => {
        ...
        options.push(
            <GridItem key="required" xs={12} sm={12} md={1}>
                <CustomInput
                    ...
                    required
                    handleChange={(event) =>
                        this.handleChange(event, formIndex, 'root')
                    }
        ...
        if (datatypes[datatype] !== undefined && datatypes[datatype].options) {
            options.push(
                datatypes[datatype].options.map((option, index) => {
                    let value =
                        defaultCondition !== undefined &&
                        defaultCondition[option.id] !== undefined
                            ? defaultCondition[option.id]
                            : ''
                    ...
                    return (
                        <GridItem key={index} xs={12} sm={12} md={2}>
                            <CustomInput
                                ...
                                condition={{
                                    errorText:
                                        datatypes[option.datatype].errorText,
                                }}
                                value={value + ''}
                                handleChange={(event) =>
                                    this.handleChange(
                ...
                <GridContainer>
                    <GridItem xs={12} sm={12} md={3}>
                        ...
                        <CustomInput
                            ...
                            handleChange={(event) =>
                                this.handleChange(event, index, 'root')
                            }
                            validateNow={this.state.validateNow}
                            changeErrorStatus={this.changeErrorStatus}
                        />
